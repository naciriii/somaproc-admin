...
<div class="">
    ...
    <div class="row">
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2>Menus <a href="{{ url('/menu/add') }}" class="btn btn-primary btn-xs"><i class="fa fa-plus"></i> Create New Menu</a></h2>
                ...
                <div class="x_content">
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><span class="glyphicon glyphicon-{{$row->icon}}"></span> {{$row->icon}}</td>
                                <td>{{$row->order}}</td>
                                <td>{{$row->name}}</td>
                                <td>{{$row->parent ? $row->parent->name : ''}}</td>
                                <td>
                                    <a href="{{ url('/menu/edit') }}/{{$row->id}}" class="btn btn-info btn-xs"><i class="fa fa-pencil" title="Edit"></i> </a>
...
<script>
	function delete_item(id){
        $('#myModal').modal('show');
        $('#confirm_delete_button').attr('href',"{{ url('/menu/delete') }}/"+id);
	}
</script>
