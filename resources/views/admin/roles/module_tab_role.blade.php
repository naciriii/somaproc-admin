...
<div class="">
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="x_panel">
                <div class="x_title">
                    <h2>{{$assignement->id ? "Update Assignement" : "Create Assignement"}}<a href="{{url('/module/tab/list')}}" class="btn btn-info btn-xs"><i class="fa fa-chevron-left"></i> Back </a></h2>
                    <div class="clearfix"></div>
                </div>
                <div class="x_content">
                    <br />
                    <form method="post" action="{{ url('/module/tab/store') }}/{{$assignement->id ? $assignement->id : 0}}" data-parsley-validate class="form-horizontal form-label-left">
						...
						<div class="form-group">
                            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="name">Tab
                            </label>
                            <div class="col-md-6 col-sm-6 col-xs-12">
                                <select id="tab" name="tab" class="form-control col-md-7 col-xs-12">
								  <option value="">Select Tab</option>
									@for($j=0;$j<count($tabs);$j++)
										<option value="{{$tabs[$j]->id}}" {{$assignement->tab_id == $tabs[$j]->id ? "selected" : ""}}>{{$tabs[$j]->name}}</option>
									@endfor
								  </select>
                            </div>
                        </div>
